                        ...
                        <form method="POST" action="{{ route('roles.update', $role->id) }}" class="space-y-4 md:space-y-6">
                            ...
                            <div>
                                ...
                                <div class="mt-2 space-y-2 flex flex-col">
                                    ...
                                    <label>
                                        <input type="checkbox" name="permission[{{$permission->id}}]" value="{{$permission->id}}" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-primary-300" {{ in_array($permission->id, $rolePermissions) ? 'checked' : ''}}>
                                    {{ $permission->name }}
                                    </label>
                                    @endforeach
